...
class ZohoTest extends WP_UnitTestCase {
	 ...
	 * @var string
	 */
	private const BACKEND_NAME = 'test-zoho-backend';
	 ...
	 * @var string
	 */
	private const CREDENTIAL_NAME = 'test-zoho-credential';
	 ...
	 * @var string
	 */
	private const BRIDGE_NAME = 'test-zoho-bridge';
	...
	public function test_addon_class_exists() {
		$this->assertTrue( class_exists( 'FORMS_BRIDGE\Zoho_Addon' ) );
		$this->assertEquals( 'Zoho', Zoho_Addon::TITLE );
		$this->assertEquals( 'zoho', Zoho_Addon::NAME );
		$this->assertEquals( '\FORMS_BRIDGE\Zoho_Form_Bridge', Zoho_Addon::BRIDGE );
	}
	...
	public function test_form_bridge_class_exists() {
		$this->assertTrue( class_exists( 'FORMS_BRIDGE\Zoho_Form_Bridge' ) );
	}
	...
	public function test_addon_get_endpoints() {
		...
		$this->assertContains( '/crm/v7/Contacts', $endpoints );
		$this->assertContains( '/crm/v7/Leads', $endpoints );
		$this->assertContains( '/crm/v7/Accounts', $endpoints );
		$this->assertContains( '/crm/v7/Deals', $endpoints );
	}
