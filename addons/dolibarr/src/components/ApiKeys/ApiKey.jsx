...
export default function ApiKey({ data, update, remove }) {
  if (data.name === "add") return <NewApiKey add={update} />;
  ...
  const keyNames = useApiKeyNames();
  const [nameConflict, setNameConflict] = useState(false);
  const handleSetName = (name) => {
    setNameConflict(name !== initialName.current && keyNames.has(name.trim()));
    setName(name);
  };
  ...
  useEffect(() => {
    ...
    timeout.current = setTimeout(() => {
      if (keyNames.has(name.trim())) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: "150px", maxWidth: "250px" }}>
          <TextControl
            label={__("Key value", "forms-bridge")}
            value={data.key}
            onChange={(key) => update({ ...data, key })}
            __nextHasNoMarginBottom
            __next40pxDefaultSize
          />
        </div>
